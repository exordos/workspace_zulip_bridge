...
class WorkspaceDiffWorker:
    ...
    def _claim_batch_size(self) -> int:
        if getattr(self, "_delivery_priority", None) == 0:
            return min(
                self._settings.workspace_sync_batch_size,
                self.LIVE_BATCH_SIZE,
            )
        if getattr(self, "_scope", "both") == "unpartitioned":
            return min(
                self._settings.workspace_sync_batch_size,
                self.UNPARTITIONED_BATCH_SIZE,
            )
        return self._settings.workspace_sync_batch_size
    ...
    async def process_once(self, client: httpx.AsyncClient) -> int:
        async with self._pool.acquire() as connection, connection.transaction():
            ...
            delivery_priority = getattr(self, "_delivery_priority", None)
